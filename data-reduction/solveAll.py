...
for root, dirnames, filenames in os.walk('/home/dokeeffe/Pictures/CalibratedLight'):
	...
	if not solved:
		...
		print 'focalLength ' + str(focalLength)
	        ...
	        print 'solve-field --no-plots --no-fits2fits --resort --skip-solved -O -L'+ str(arcSecPerPixel*.95) +  '-H' +  str(arcSecPerPixel*1.05) + '-u app -3 ' +  str(ra) + '-4' + str(dec) + '-5 30' +  temp_file
	        subprocess.call(['solve-field', '--no-plots', '--no-fits2fits', '--resort', '--skip-solved', '-O', '-L', str(arcSecPerPixel*.95), '-H', str(arcSecPerPixel*1.05),'-u','app','-3', str(ra), '-4', str(dec), '-5' ,'30', temp_file] )
